        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="relative group">
            ...
            <div className="relative aspect-square w-full rounded-3xl overflow-hidden bg-zinc-900 flex items-center justify-center border border-white/5 shadow-2xl">
              <img alt={product.name} className="w-4/5 h-4/5 object-contain transition-transform duration-700 hover:scale-110" src={product.image} />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-8">
            <div className="space-y-4 text-center lg:text-left">
              ...
              <p className="text-3xl font-black text-primary/90 italic tracking-tighter">${product.price.toFixed(2)}</p>
            ...
            <div className="grid grid-cols-2 gap-4 py-8 border-y border-white/10">
              ...
              <div className="flex items-center gap-4">
                <div className="size-10 rounded-full bg-primary/10 flex items-center justify-center">
                  <span className="material-symbols-outlined text-primary text-xl">texture</span>
                </div>
                <div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center mt-4">
              <div className="flex items-center bg-zinc-900 border border-white/10 rounded px-4 h-16 w-full sm:w-auto">
                <button 
                  disabled={isAdding}
                  onClick={() => setQuantity(q => Math.max(1, q - 1))}
                  className="w-10 h-10 flex items-center justify-center hover:text-primary transition-colors disabled:opacity-30">
              ...
              <button 
                ...
                className={`flex-1 w-full h-16 gold-gradient text-black font-black uppercase tracking-widest rounded-lg flex items-center justify-center gap-3 transition-all duration-300 ${isAdding ? 'opacity-80 cursor-wait scale-[0.98]' : 'hover:scale-[1.02] active:scale-95'}`}>
                {isAdding ? (
        ...
        <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-4 gap-12 text-center">
            {[
              ...
              <div key={i} className="flex flex-col items-center gap-4">
                <span className="material-symbols-outlined text-primary text-4xl">{stat.icon}</span>
